...
/** When Postgres/bookings are unavailable — still offer slot grid from the static catalog. */
export const getAvailabilityFromCatalogOnly = async ({ serviceId, from, to }) => {
  const catalog = await loadFallbackCatalog();
  const service = catalog.find((s) => Number(s.id) === Number(serviceId));
  if (!service || service.is_active === false) {
    const err = new Error('Service not found');
    err.status = 404;
    throw err;
  ...
  const data = dates.map((date) => ({
    date,
    slots: buildDaySlots(date, empty, now, tz),
  }));
