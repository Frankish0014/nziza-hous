  ...
  return (
    <main className="mx-auto max-w-7xl space-y-8 px-4 py-12">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        ...
        <Link
          href="/admin/bookings"
          className="btn-primary inline-flex w-fit items-center justify-center rounded-full px-5 py-2.5 text-sm font-semibold"
        >
          ...
          <div className="mt-4 space-y-3">
            {bookings.map((item) => (
              <div key={item.id} className="rounded-xl border border-stone-200 bg-white/80 p-3">
                <p className="font-medium text-stone-900">{item.service_name}</p>
                <p className="text-sm text-stone-700">
                  {item.full_name || item.user_name || 'Guest'}
                  {item.email ? ` · ${item.email}` : ''}
                </p>
                <p className="text-sm text-stone-600">
                  {String(item.booking_date).slice(0, 10)} | {item.time_slot} | {item.status}
                </p>
